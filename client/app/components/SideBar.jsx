...
export default function Sidebar({children}) {
  ...
  if (user.role === 'employee') {
    ...
    links.push({ href: '/leave-request', label: 'Request Leave' });
  }

  if (user.role === 'manager' || user.role === 'admin') {
    links.push({ href: '/adminDashboard', label: 'Dashboard' });
    links.push({ href: '/calendar', label: 'Calendar' });
    links.push({ href: '/manage-leave', label: 'Approve Leave' });
    links.push({ href: '/manage-employee', label: 'Manage Employees' });
